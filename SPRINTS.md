...
- [x] Add idempotency: dedupe on provider message id so retried webhooks don't double-process — `escalate_to_human`, `create_return_request`, `create_refund_request`, and `create_draft` all have DB-level unique-constraint + conflict handling; WhatsApp send uses an `Idempotency-Key` header keyed off the inbound message id. Still open: this covers duplicate *sends and writes*, not duplicate *inbound processing* — nothing yet stops `receive_message` from calling `handle_incoming` twice if Meta redelivers the same webhook before your server acks it
