...
- [x] Add structured logging (or LangSmith tracing) covering: which specialists the orchestrator called and in what order, each specialist's own tool calls, and the final response — all tagged with `thread_id` — LangSmith handles the agent/tool-call trace; `structlog` (via `logger.py`) now covers writes, failures, and adapter lifecycle events across `agent.py`, `gmail_adapter.py`, `whatsapp_adapter.py`, `voice_adapter.py`
- [x] Add retries + timeouts (e.g. `tenacity`) on every external call — LLM calls (all 5), DB reads/writes in `agent.py`'s tools, and Gmail reads/mark-as-read in `gmail_adapter.py` done; Gmail `create_draft`'s MCP call now has its own retry scoped to `httpx.ReadTimeout/ConnectError/RemoteProtocolError`, WhatsApp's send is wrapped with status-code-aware retry (`raise_for_status()` inside the retried function so 5xx/429 actually trigger a retry, 409 does not), and the voice adapter's OpenAI Realtime connection retries transient handshake/connection failures with backoff before the call proceeds
- [x] Add per-customer/thread rate limiting — Redis-backed fixed-window counter (`INCR` + conditional `EXPIRE`) in `handle_incoming`, namespaced per channel+thread, fails open if Redis is unreachable
- [x] Move all secrets to env vars / a secrets manager; double-check nothing is hardcoded or committed — audited: every key/password/token in `src/` flows through `settings.*`, and `.env`/`gmail_token.json`/`client_secret.json` are gitignored and confirmed never committed (checked full git history, not just working tree). Non-secret hardcoding still present (DB host/name duplicated in 3 files, a placeholder ngrok URL in `voice_adapter.py`) — noted but doesn't block this item; the DB host/name gets addressed naturally in Sprint 12's multi-tenancy retrofit
- [x] Add idempotency: dedupe on provider message id so retried webhooks don't double-process — `escalate_to_human`, `create_return_request`, `create_refund_request`, and `create_draft` all have DB-level unique-constraint + conflict handling; WhatsApp send uses an `Idempotency-Key` header keyed off the inbound message id. Inbound dedup also done: `receive_message` now fast-acks Meta immediately (signature verify + payload parse only, before any LLM/send work) and processes `handle_incoming` + the reply send in a `BackgroundTasks`-scheduled function, with a Redis `SET NX EX` gate (7-day TTL, matching Meta's own retry window) on the inbound message id so a webhook redelivery is a no-op instead of a second full agent turn — fails open (treats Redis errors as "not a duplicate") so a Redis outage doesn't block WhatsApp entirely
- [x] Add a lightweight `/health` endpoint (used by the Docker healthcheck and k8s probes in Sprint 10b) — `/healthz` on `main.py`, returns a static `{"status": "healthy"}`
