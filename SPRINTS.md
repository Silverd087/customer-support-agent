...
- [ ] Add retries + timeouts (e.g. `tenacity`) on every external call — LLM calls (all 5), DB reads/writes in `agent.py`'s tools, and Gmail reads/mark-as-read in `gmail_adapter.py` done; still open: no retry wrapping on the Gmail `create_draft` MCP call or the WhatsApp `requests.post` send itself (both now have idempotency protection, but a transient failure still isn't retried, just logged), and the voice adapter's Realtime API connection has no retry-with-backoff on initial connect
...
- [ ] Move all secrets to env vars / a secrets manager; double-check nothing is hardcoded or committed — `.env` pattern in place project-wide, still needs a final audit pass to confirm nothing slipped through
